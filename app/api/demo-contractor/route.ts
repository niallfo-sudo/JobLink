...
import { DEMO_CONTRACTOR_COOKIE, isDemoContractorEmail } from "../../contractor-demo";
...
export async function GET() {
  ...
  const rows = await getDb().select({ ownerEmail: contractorProfiles.ownerEmail, businessName: contractorProfiles.businessName, primaryService: contractorProfiles.primaryService, homeBase: contractorProfiles.homeBase, verificationStatus: contractorProfiles.verificationStatus })
    ...
    .where(or(eq(contractorProfiles.ownerEmail, identity.email), eq(contractorProfiles.ownerEmail, "demo-north-beam@joblink.demo"), eq(contractorProfiles.ownerEmail, "demo-hamilton-climate@joblink.demo"), eq(contractorProfiles.ownerEmail, "demo-hamilton-plumbing@joblink.demo"), eq(contractorProfiles.ownerEmail, "demo-citywide-painting@joblink.demo")))
...
export async function POST(request: Request) {
  ...
  if (!isDemoContractorEmail(selectedEmail)) return Response.json({ error: "Choose a JobLink demo company" }, { status: 400 });
  ...
  if (!profile) return Response.json({ error: "That demo company is unavailable" }, { status: 404 });
