...
export async function GET() {
  ...
  const rows = await getDb().select({ id: quotes.id, jobId: quotes.jobId, externalId: jobRequests.externalId, title: jobRequests.title, category: jobRequests.category, description: jobRequests.description, size: jobRequests.size, timeline: jobRequests.timeline, postalCode: jobRequests.postalCode, amountCents: quotes.amountCents, message: quotes.message, initialMinCents: quotes.initialMinCents, initialMaxCents: quotes.initialMaxCents, quoteAccuracyDelta: quotes.quoteAccuracyDelta, quoteAccuracyStatus: quotes.quoteAccuracyStatus, availableAt: quotes.availableAt, estimatedStartAt: quotes.estimatedStartAt, estimatedFinishAt: quotes.estimatedFinishAt, finalStartAt: quotes.finalStartAt, status: quotes.status, onsiteVisitAt: quotes.onsiteVisitAt, onsitePreferences: quotes.onsitePreferences, onsiteProposals: quotes.onsiteProposals, workDescription: quotes.workDescription, materials: quotes.materials, measurements: quotes.measurements, depositCents: quotes.depositCents, progressCents: quotes.progressCents, progressRequirement: quotes.progressRequirement, completionCents: quotes.completionCents, finalOptions: quotes.finalOptions, selectedFinalOptionId: quotes.selectedFinalOptionId, createdAt: quotes.createdAt }).from(quotes).innerJoin(jobRequests, eq(quotes.jobId, jobRequests.id)).where(eq(quotes.contractorEmail, user.email)).orderBy(desc(quotes.createdAt));
