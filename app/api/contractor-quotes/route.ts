...
export async function GET() {
  ...
  const rows = await getDb().select({ id: quotes.id, jobId: quotes.jobId, externalId: jobRequests.externalId, title: jobRequests.title, category: jobRequests.category, amountCents: quotes.amountCents, initialMinCents: quotes.initialMinCents, initialMaxCents: quotes.initialMaxCents, quoteAccuracyDelta: quotes.quoteAccuracyDelta, quoteAccuracyStatus: quotes.quoteAccuracyStatus, availableAt: quotes.availableAt, estimatedStartAt: quotes.estimatedStartAt, estimatedFinishAt: quotes.estimatedFinishAt, status: quotes.status, onsiteVisitAt: quotes.onsiteVisitAt, workDescription: quotes.workDescription, materials: quotes.materials, measurements: quotes.measurements, depositCents: quotes.depositCents, progressCents: quotes.progressCents, completionCents: quotes.completionCents, finalOptions: quotes.finalOptions, selectedFinalOptionId: quotes.selectedFinalOptionId, createdAt: quotes.createdAt }).from(quotes).innerJoin(jobRequests, eq(quotes.jobId, jobRequests.id)).where(eq(quotes.contractorEmail, user.email)).orderBy(desc(quotes.createdAt));
