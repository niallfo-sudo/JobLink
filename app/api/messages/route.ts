...
import { jobRequests, messages } from "../../../db/schema";
import { getChatGPTUser } from "../../chatgpt-auth";

async function ownedJob(jobId: number, email: string) {
...
export async function GET(request: Request) {
  ...
  try {
    if (!(await ownedJob(jobId, user.email))) return Response.json({ error: "Job not found" }, { status: 404 });
