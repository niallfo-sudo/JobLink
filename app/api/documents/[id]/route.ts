...
import { agreementSignatures, documentRecords } from "../../../../db/schema";
...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const signatures = document.documentType === "service_agreement" ? await getDb().select().from(agreementSignatures).where(eq(agreementSignatures.documentId, document.id)) : [];
    ...
    const homeownerSignature = signatures.find((signature) => signature.signerRole === "homeowner");
    const contractorSignature = signatures.find((signature) => signature.signerRole === "contractor");
    const renderedHtml = html
      .replace("Homeowner signature / date", homeownerSignature ? `Homeowner signed: ${escapeHtml(homeownerSignature.signerName)}<br>${escapeHtml(homeownerSignature.signedAt.toLocaleString("en-CA"))}` : "Homeowner signature / date — pending")
      .replace("Contractor signature / date", contractorSignature ? `Contractor signed: ${escapeHtml(contractorSignature.signerName)}<br>${escapeHtml(contractorSignature.signedAt.toLocaleString("en-CA"))}` : "Contractor signature / date — pending");
    return new Response(renderedHtml, { headers: { "Content-Type": "text/html; charset=utf-8", "Content-Security-Policy": "default-src 'none'; style-src 'unsafe-inline'; base-uri 'none'; frame-ancestors 'self'", "X-Content-Type-Options": "nosniff", "Content-Disposition": `inline; filename="${document.externalId}.html"` } });
  } catch (error) {
