...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const now = new Date();
    const [signature] = await db.insert(agreementSignatures).values({ documentId, jobId: document.jobId, signerEmail: user.email, signerRole, signerName, consentText, signingMethod: "account_attestation", userAgent: request.headers.get("user-agent")?.slice(0, 500) || "", signedAt: now }).onConflictDoUpdate({ target: [agreementSignatures.documentId, agreementSignatures.signerRole], set: { signerEmail: user.email, signerName, consentText, signingMethod: "account_attestation", userAgent: request.headers.get("user-agent")?.slice(0, 500) || "", signedAt: now } }).returning();
