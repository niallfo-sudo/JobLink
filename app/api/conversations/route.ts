...
export async function GET() {
  ...
  try {
    ...
    const [owned, quoted] = await Promise.all([
      ...
      db.select({ job: jobRequests }).from(quotes).innerJoin(jobRequests, eq(quotes.jobId, jobRequests.id)).where(eq(quotes.contractorEmail, user.email)).orderBy(desc(jobRequests.updatedAt)).limit(20),
    ]);
