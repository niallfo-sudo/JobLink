...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [milestones, reviews, attachments] = await Promise.all([
      ...
      jobIds.length ? getDb().select({ id: jobAttachments.id, jobId: jobAttachments.jobId, milestoneId: jobAttachments.milestoneId, filename: jobAttachments.filename, contentType: jobAttachments.contentType, sizeBytes: jobAttachments.sizeBytes, kind: jobAttachments.kind, stage: jobAttachments.stage, createdAt: jobAttachments.createdAt }).from(jobAttachments).where(inArray(jobAttachments.jobId, jobIds)) : [],
    ]);
    ...
    return Response.json({ payments: rows.map((row) => ({ ...row.payment, ...row.job, completionReviewSubmitted: reviewedJobIds.has(row.payment.jobId), preWorkPhotoCount: attachments.filter((attachment) => attachment.jobId === row.payment.jobId && attachment.stage === "pre_work" && attachment.kind === "image").length, preWorkPhotos: attachments.filter((attachment) => attachment.jobId === row.payment.jobId && attachment.stage === "pre_work" && attachment.kind === "image").map(toAttachment), viewerRole: scope === "contractor" ? "contractor" : scope === "homeowner" ? "homeowner" : row.payment.ownerEmail === user.email ? "homeowner" : "contractor", milestones: milestones.filter((milestone) => milestone.paymentId === row.payment.id).map((milestone) => ({ ...milestone, progressPhotos: attachments.filter((attachment) => attachment.milestoneId === milestone.id && attachment.stage === "progress" && attachment.kind === "image").map(toAttachment) })) })) });
  } catch (error) {
