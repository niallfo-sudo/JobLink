...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [quote] = await db.insert(quotes).values({ jobId, contractorEmail: user.email, contractorName: profile.businessName, amountCents, message: payload.message?.trim() || "Initial estimate. A final written scope follows an on-site verification.", availableAt: payload.availableAt?.trim() || "On-site availability to be confirmed" }).onConflictDoUpdate({
      target: [quotes.jobId, quotes.contractorEmail],
      set: { amountCents, message: payload.message?.trim() || "Initial estimate. A final written scope follows an on-site verification.", availableAt: payload.availableAt?.trim() || "On-site availability to be confirmed", status: "submitted", onsiteVisitAt: null, workDescription: "", materials: "", measurements: "", depositCents: 0, progressCents: 0, completionCents: 0, finalizedAt: null, createdAt: new Date() },
    }).returning();
    await db.insert(jobEvents).values({ jobId, eventType: "quote_submitted", label: `Initial estimate received from ${profile.businessName}`, metadata: JSON.stringify({ quoteId: quote.id, amountCents }) });
    await notify(job.ownerEmail, { jobId, type: "quote_received", title: "New initial estimate", body: `${profile.businessName} submitted an estimate for ${job.externalId}.` });
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (payload.action === "accept_final") {
      ...
      await db.insert(paymentRecords).values({ jobId, quoteId, ownerEmail: user.email, contractorEmail: quote.contractorEmail, contractorName: quote.contractorName, subtotalCents: quote.amountCents, customerFeeCents, totalCents: quote.amountCents + customerFeeCents, contractorPayoutCents: quote.amountCents, status: "demo_pending", processor: "demo" }).onConflictDoUpdate({ target: paymentRecords.jobId, set: { quoteId, contractorEmail: quote.contractorEmail, contractorName: quote.contractorName, subtotalCents: quote.amountCents, customerFeeCents, totalCents: quote.amountCents + customerFeeCents, contractorPayoutCents: quote.amountCents, status: "demo_pending", processor: "demo", updatedAt: new Date() } });
