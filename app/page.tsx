...
type View = "discover" | "request" | "matches" | "tracking" | "contractor" | "account" | "trust" | "help" | "how" | "onboarding" | "silent" | "emergency" | "admin";
...
type ContractorProfile = { businessName: string; legalName: string; phone: string; businessAddress: string; yearsInBusiness: number; about: string; primaryService: string; services: string[]; approvedServices: string[]; homeBase: string; serviceRadiusKm: number; teamSize: number; emergencyAvailable: boolean; acceptingWork: boolean; plan: "starter" | "growth" | "pro"; subscriptionStatus: string; payoutsEnabled: boolean; verificationStatus: string; demoMetrics?: { averageRating: number; jobLinkScore: number; completedJobs: number; quoteRating: number; releasedPayoutCents: number } | null };
type JobAttachment = { id: number; filename: string; contentType: string; sizeBytes: number; kind: "image" | "video"; stage?: string; milestoneId?: number | null; url: string; createdAt: string | number };
type PaymentMilestone = { id: number; paymentId: number; label: string; milestoneType: string; amountCents: number; status: string; proofNote: string; operationsNote?: string; proofSubmittedAt?: string | number | null; homeownerApprovedAt?: string | number | null; releasedAt?: string | number | null; progressPhotos?: JobAttachment[] };
type PaymentReview = PaymentMilestone & { externalId: string; jobTitle: string; contractorName: string; operationsNote?: string; operationsReviewedBy?: string | null; operationsReviewedAt?: string | number | null; preWorkPhotos?: JobAttachment[]; progressPhotos?: JobAttachment[] };
type PaymentRecord = { id: number; jobId: number; externalId: string; title: string; contractorName: string; subtotalCents: number; customerFeeCents: number; totalCents: number; contractorPayoutCents: number; releasedCents: number; status: string; processor: string; viewerRole: "homeowner" | "contractor"; completionReviewSubmitted?: boolean; preWorkPhotoCount?: number; preWorkPhotos?: JobAttachment[]; milestones?: PaymentMilestone[]; createdAt: string | number };
...
type GeneratedDocument = { id: number; jobId: number; quoteId?: number | null; jobTitle: string; jobNumber: string; documentType: string; title: string; status: string; viewerRole?: "homeowner" | "contractor"; signatures?: DocumentSignature[]; preWorkPhotoCount?: number; preWorkPhotos?: JobAttachment[]; createdAt: string | number };
type DemoLifecycleAction = "seed" | "sign" | "fund" | "approve_all";
type ActionFeedback = { tone: "success" | "error"; message: string };
type DemoLifecycleProgress = { seeded: boolean; signed: boolean; funded: boolean; approved: boolean };
...
function ImageThumbnailStrip({ images, label }: { images: JobAttachment[]; label: string }) {
  if (!images.length) return null;
  return <div className="image-thumbnail-strip"><p>{label}</p><div>{images.map((image) => <a key={image.id} href={image.url} target="_blank" rel="noreferrer" title={`Open ${image.filename}`}><img src={image.url} alt={image.filename} loading="lazy" /><span>{image.filename}</span></a>)}</div></div>;
}

function QuoteScorePanel({ contractor }: { contractor: ContractorComparison | null | undefined }) {
  const score = contractor?.jobLinkScore;
  const rating = contractor?.averageRating;
  const rows = [
    ["Verified reviews", contractor?.reviewCount ? String(contractor.reviewCount) : "Building"],
    ["Quality score", contractor?.qualityScore ?? "Building"],
    ["Completion reliability", contractor?.completionReliability ?? "Building"],
    ["Review completion", contractor?.reviewCompletion ?? "Building"],
    ["Timeline reliability", contractor?.timelineReliability ?? "Building"],
    ["Quote Rating", contractor?.quoteRating ?? "Building"],
  ] as const;
  return <section className="quote-score-panel" aria-label={`${contractor?.primaryService || "Contractor"} ratings`}><header><div><p>JobLink Score</p><b>{score ?? "New"}<small>{typeof score === "number" ? "/100" : " - Building"}</small></b></div><span>{rating ? `${rating.toFixed(1)} stars` : "Building"}</span></header><div className="quote-score-bar"><i style={{ width: `${score ?? 0}%` }} /></div><dl>{rows.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><p className="quote-score-note">Verified work, review completion, on-time completion and accurate initial price ranges help homeowners compare reliable quotes.</p></section>;
}
...
function BeforeWorkPhotoUpload({ jobId, photoCount, onUploaded }: { jobId: number; photoCount: number; onUploaded: () => void }) {
  ...
  const inputId = `before-work-photos-${jobId}`;
  return <section className="before-work-photo-upload"><b>Before-work photos required</b><p>Upload clear images of the work area before signing or funding. They are kept with the JobLink job record for scope and completion verification.</p>{photoCount > 0 ? <span className="agreement-complete">{photoCount} before-work photo{photoCount === 1 ? "" : "s"} recorded ✓</span> : <div className="before-photo-action"><input id={inputId} className="before-photo-file-input" type="file" accept="image/jpeg,image/png,image/webp,image/heic" multiple disabled={uploading} onChange={(event) => void upload(event.target.files)} /><label htmlFor={inputId} className="primary-action upload-before-photos-button">{uploading ? "Uploading before-work photos…" : "Upload required project before photos →"}</label><small>Choose up to 5 JPG, PNG, WebP or HEIC images.</small></div>}{error && <small className="quote-submit-error">{error}</small>}</section>;
}

function LegacyPaymentReleaseWorkflow({ payment, actionId, onFund, onSubmitProof, onApprove, onOpenReview }: { payment: PaymentRecord; actionId: string | null; onFund: (payment: PaymentRecord) => void; onSubmitProof: (payment: PaymentRecord, milestone: PaymentMilestone, proof: string) => void; onApprove: (payment: PaymentRecord, milestone: PaymentMilestone) => void; onOpenReview: (payment: PaymentRecord) => void }) {
...
function PaymentReleaseWorkflow({ payment, actionId, onFund, onSubmitProof, onApprove, onOpenReview }: { payment: PaymentRecord; actionId: string | null; onFund: (payment: PaymentRecord) => void; onSubmitProof: (payment: PaymentRecord, milestone: PaymentMilestone, proof: string, files: File[]) => void; onApprove: (payment: PaymentRecord, milestone: PaymentMilestone) => void; onOpenReview: (payment: PaymentRecord) => void }) {
  const [proofs, setProofs] = useState<Record<number, string>>({});
  const [progressFiles, setProgressFiles] = useState<Record<number, File[]>>({});
  const milestones = payment.milestones ?? [];
  return <section className="payment-release-workflow">
    <div className="payment-release-header"><div><p className="aside-label">Protected release plan</p><h3>{payment.externalId} - ${((payment.releasedCents || 0) / 100).toLocaleString()} released</h3><p>Full job amount is held first; each contractor payment needs evidence and homeowner approval.</p></div><span className={`payment-status payment-${payment.status}`}>{payment.status.replaceAll("_", " ")}</span></div>
    <ImageThumbnailStrip images={payment.preWorkPhotos ?? []} label="Original before-work photos" />
    {payment.viewerRole === "homeowner" && payment.status === "demo_pending" && <button className="primary-action" disabled={actionId === `fund-${payment.id}`} onClick={() => onFund(payment)}>{actionId === `fund-${payment.id}` ? "Funding..." : `Fund full job total $${(payment.totalCents / 100).toLocaleString()} (demo)`}</button>}
    {milestones.length ? <div className="payment-milestone-list">{milestones.map((milestone) => {
      const inputId = `progress-photo-${milestone.id}`;
      const selectedFiles = progressFiles[milestone.id] ?? [];
      const recordedPhotos = milestone.progressPhotos ?? [];
      const photoReady = recordedPhotos.length > 0 || selectedFiles.length > 0;
      return <article key={milestone.id}><div><b>{milestone.label}</b><span>${(milestone.amountCents / 100).toLocaleString()} - {milestone.status.replaceAll("_", " ")}</span>{milestone.proofNote && <p><strong>Contractor proof:</strong> {milestone.proofNote}</p>}{milestone.operationsNote && <p><strong>Operations review:</strong> {milestone.operationsNote}</p>}<ImageThumbnailStrip images={recordedPhotos} label="Submitted progress photos" /></div>{payment.viewerRole === "contractor" && milestone.status === "demo_held" && <div className="payment-proof-entry"><textarea value={proofs[milestone.id] ?? ""} onChange={(event) => setProofs((current) => ({ ...current, [milestone.id]: event.target.value }))} placeholder="Describe the completed work shown in the required progress photos." /><input id={inputId} className="before-photo-file-input" type="file" accept="image/jpeg,image/png,image/webp,image/heic" multiple onChange={(event) => setProgressFiles((current) => ({ ...current, [milestone.id]: Array.from(event.target.files ?? []).slice(0, 5) }))} /><label htmlFor={inputId} className={photoReady ? "progress-photo-ready" : "progress-photo-required"}>{recordedPhotos.length ? `Photo evidence on file (${recordedPhotos.length})` : selectedFiles.length ? `${selectedFiles.length} progress photo${selectedFiles.length === 1 ? "" : "s"} selected` : "Upload progress photos (required)"}</label><small>Operations compares these with the original before-work photos before funds can be released.</small><button disabled={actionId === `proof-${milestone.id}` || !photoReady} onClick={() => onSubmitProof(payment, milestone, proofs[milestone.id] ?? "", selectedFiles)}>{actionId === `proof-${milestone.id}` ? "Submitting..." : "Submit proof for Operations review"}</button></div>}{payment.viewerRole === "homeowner" && milestone.status === "proof_submitted" && (milestone.milestoneType === "completion" && !payment.completionReviewSubmitted ? <div className="final-release-review-lock"><b>Final release requires your completed-job review.</b><p>Answer every review question and add written feedback before this final payment can be released.</p><button className="primary-action" onClick={() => onOpenReview(payment)}>Complete required review</button></div> : <button className="primary-action" disabled={actionId === `approve-${milestone.id}`} onClick={() => onApprove(payment, milestone)}>{actionId === `approve-${milestone.id}` ? "Approving..." : "Approve and release (demo)"}</button>)}</article>;
    })}</div> : <p className="quote-panel-state">Payment checkpoints will appear after the final quote is accepted.</p>}
  </section>;
}

function DemoLifecycleControls({ jobs, documents, payments, actionId, feedback, progress, onRun }: { jobs: PersistedJob[]; documents: GeneratedDocument[]; payments: PaymentRecord[]; actionId: string | null; feedback: Record<number, ActionFeedback | undefined>; progress: Record<number, DemoLifecycleProgress | undefined>; onRun: (job: PersistedJob, action: DemoLifecycleAction) => void }) {
  if (!jobs.length) return null;
  const paymentFunded = (status?: string) => ["demo_held", "demo_partially_released", "demo_released"].includes(status ?? "");
  return <section className="demo-lifecycle-panel"><div><p className="aside-label">Demo job simulator</p><h3>Create sample documents, signatures and payment activity.</h3><p>Every action below is clearly marked as a simulation. It creates no legal signature, charge, transfer or real approval.</p></div><div className="demo-lifecycle-list">{jobs.map((job) => { const agreement = documents.find((document) => document.jobId === job.id && document.documentType === "service_agreement"); const payment = payments.find((record) => record.jobId === job.id); const persistedProgress = { seeded: Boolean(agreement && payment), signed: agreement?.status === "fully_signed", funded: paymentFunded(payment?.status), approved: payment?.status === "demo_released" }; const currentProgress = { seeded: persistedProgress.seeded || progress[job.id]?.seeded === true, signed: persistedProgress.signed || progress[job.id]?.signed === true, funded: persistedProgress.funded || progress[job.id]?.funded === true, approved: persistedProgress.approved || progress[job.id]?.approved === true }; const steps: Array<{ action: DemoLifecycleAction; label: string; done: boolean; blocked: boolean }> = [{ action: "seed", label: "Create demo records", done: currentProgress.seeded, blocked: false }, { action: "sign", label: "Simulate signatures", done: currentProgress.signed, blocked: !currentProgress.seeded }, { action: "fund", label: "Simulate funding", done: currentProgress.funded, blocked: !currentProgress.signed }, { action: "approve_all", label: "Simulate approvals", done: currentProgress.approved, blocked: !currentProgress.funded }]; const nextStep = steps.find((step) => !step.done); return <article key={`demo-${job.id}`}><div><b>{job.title}</b><small>{job.externalId} · {job.status.replaceAll("_", " ")}</small></div><div className="demo-step-actions">{steps.map((step, index) => <button key={step.action} className={step.done ? "complete" : ""} disabled={actionId !== null || step.done || step.blocked} onClick={() => onRun(job, step.action)}>{step.done ? `✓ ${index + 1}. ${step.label}` : actionId === `${step.action}-${job.id}` ? `${step.label}…` : `${index + 1}. ${step.label}`}</button>)}</div><div className="demo-lifecycle-status">{feedback[job.id] && <p className={feedback[job.id]?.tone === "error" ? "error" : "success"}>{feedback[job.id]?.message}</p>}{nextStep ? <small><b>Next required demo action:</b> {nextStep.blocked ? "Complete the earlier step first." : nextStep.label}</small> : <small className="success"><b>Complete:</b> all four demo records are visible below.</small>}</div></article>; })}</div></section>;
}

function AgreementSignatureWorkflow({ document, actionId, onSign }: { document: GeneratedDocument; actionId: number | null; onSign: (document: GeneratedDocument, signerName: string, consent: boolean) => Promise<ActionFeedback> }) {
  ...
  const needsBeforePhotos = role === "homeowner" && (document.preWorkPhotoCount ?? 0) < 1;
  const canSubmit = !needsBeforePhotos && consent && signerName.trim().length >= 2;
  async function submit() { const result = await onSign(document, signerName, consent); setFeedback(result); }
  return <section className="agreement-signature-workflow"><div className="agreement-signature-header"><div><p className="aside-label">Service agreement</p><h3>{document.jobNumber} · signature status</h3><p>Review the final scope, materials, payment checkpoints and job terms before signing.</p></div><button onClick={() => window.open(`/api/documents/${document.id}`, "_blank", "noopener,noreferrer")}>Open agreement ↗</button></div><div className="signature-status-grid"><span className={homeowner ? "signed" : "pending"}>Homeowner: {homeowner ? `${homeowner.signerName} · signed` : "pending"}</span><span className={contractor ? "signed" : "pending"}>Contractor: {contractor ? `${contractor.signerName} · signed` : "pending"}</span></div>{document.status === "fully_signed" ? <p className="agreement-complete">Both parties have signed. The contractor can schedule work after full-job funding is confirmed.</p> : signedByViewer ? <p className="agreement-complete">You signed on {new Date(signedByViewer.signedAt).toLocaleString()}. Waiting for the other party.</p> : <div className="agreement-sign-form"><div className="agreement-requirements"><b>Complete the required items before signing</b><span className={signerName.trim().length >= 2 ? "complete" : "required"}>{signerName.trim().length >= 2 ? "✓ Legal name entered" : "1. Enter your full legal name"}</span><span className={consent ? "complete" : "required"}>{consent ? "✓ Electronic-signature consent confirmed" : "2. Confirm electronic-signature consent"}</span>{role === "homeowner" && <span className={needsBeforePhotos ? "required" : "complete"}>{needsBeforePhotos ? "3. Upload at least one before-work photo above" : "✓ Before-work photo recorded"}</span>}</div><label>Legal name<input value={signerName} onChange={(event) => { setSignerName(event.target.value); setFeedback(null); }} placeholder="Enter your full legal name" /></label><label className="signature-consent"><input type="checkbox" checked={consent} onChange={(event) => { setConsent(event.target.checked); setFeedback(null); }} /> <span>I reviewed this agreement and intend to sign it electronically. JobLink will retain this signature record.</span></label><button className="primary-action" disabled={actionId === document.id || !canSubmit} onClick={() => void submit()}>{actionId === document.id ? "Recording signature…" : needsBeforePhotos ? "Upload before-work photos to sign" : "Sign service agreement"}</button>{feedback && <p className={feedback.tone === "error" ? "workflow-feedback error" : "workflow-feedback success"}>{feedback.message}</p>}<small>For court use or specific trade requirements, have Ontario counsel review your final agreement and signing process.</small></div>}</section>;
}

function LegacyOperationsPaymentReviewQueue({ reviews, actionId, onReview }: { reviews: PaymentReview[]; actionId: string | null; onReview: (review: PaymentReview, decision: "hold" | "clear", note: string) => void }) {
...
function OperationsPaymentReviewQueue({ reviews, actionId, onReview }: { reviews: PaymentReview[]; actionId: string | null; onReview: (review: PaymentReview, decision: "hold" | "clear", note: string) => void }) {
  const [notes, setNotes] = useState<Record<number, string>>({});
  return <section className="operations-payment-review"><div className="operations-payment-review-head"><div><p className="step-kicker">Payment oversight</p><h2>Proof review queue.</h2><p>Compare the contractor's progress images against the original documented condition before clearing a payment for the homeowner.</p></div><span>{reviews.length} open</span></div>{reviews.length ? <div className="operations-payment-review-list">{reviews.map((review) => <article key={review.id}><div><b>{review.externalId} - {review.label}</b><small>{review.jobTitle} - {review.contractorName} - ${(review.amountCents / 100).toLocaleString()}</small><p><strong>Submitted proof:</strong> {review.proofNote}</p><ImageThumbnailStrip images={review.preWorkPhotos ?? []} label="Original before-work photos" /><ImageThumbnailStrip images={review.progressPhotos ?? []} label="Contractor progress photos" />{review.operationsNote && <p><strong>Last Operations note:</strong> {review.operationsNote}</p>}</div><div className="operations-payment-review-actions"><textarea value={notes[review.id] ?? ""} onChange={(event) => setNotes((current) => ({ ...current, [review.id]: event.target.value }))} placeholder="Reason for hold or clearance" /><div>{review.status === "proof_submitted" ? <button className="reject" disabled={actionId === `hold-${review.id}`} onClick={() => onReview(review, "hold", notes[review.id] ?? "")}>{actionId === `hold-${review.id}` ? "Saving..." : "Place on hold"}</button> : <button className="approve" disabled={actionId === `clear-${review.id}`} onClick={() => onReview(review, "clear", notes[review.id] ?? "")}>{actionId === `clear-${review.id}` ? "Saving..." : "Clear for homeowner"}</button>}</div></div></article>)}</div> : <div className="operations-no-results"><b>No payment proof needs Operations attention.</b><p>Submitted proof appears here with original and progress photos until the homeowner approves it or Operations places it on hold.</p></div>}</section>;
}
...
const baseHelpFaqs = helpFaqsRaw.map((item) => item.question === "Does posting a job cost anything?"
...
const helpFaqs = [
  ...baseHelpFaqs.map((item) => item.question === "How is the JobLink Score calculated?"
    ? { ...item, answer: "The JobLink Score begins as New - Building until the first verified completed-job review is received. It then reflects verified work, not unverified public reviews: review quality (45%), completion of accepted jobs (25%), required homeowner-review completion (10%), and on-time completion against the finalized completion target (20%). Completing on or before the agreed target earns full timeline credit; later completion earns less." }
    : item.question === "How does Quote Rating work?"
      ? { ...item, answer: "Each initial quote includes an honest minimum-to-maximum range. Quote Rating recognizes contractors whose finalized on-site quotes consistently remain within the range they first provided. It helps homeowners compare reliable estimates instead of being drawn to artificially low opening bids." }
      : item.question === "What happens if something goes wrong?"
        ? { ...item, answer: "Keep communication, the agreement, photos, progress updates, and payment actions inside JobLink. Operations can review the documented job record and pause unreleased milestone funds while the issue is reviewed." }
        : item),
  { topic: "homeowners", question: "Can I compare quotes before I commit?", answer: "Yes. Initial estimates are non-binding. You can compare qualified contractors, request on-site verification from more than one, and review the finalized scope and agreement before you select a contractor." },
  { topic: "professionals", question: "Why do I need to give an honest initial range?", answer: "Homeowners use the opening range to compare realistic options. Quote Rating recognizes contractors whose final verified quote stays aligned with that range, helping accurate estimates rank above artificially low opening bids." },
  { topic: "payments", question: "How will payment protection work at launch?", answer: "Live payments are designed to be handled by a licensed third-party payment provider. Funds will follow the signed agreement and be released by agreed milestones after documented progress, homeowner approval, and Operations review where required. The current payment workflow is a demo only." },
  { topic: "trust", question: "How does JobLink help protect against incomplete work?", answer: "The final scope, materials, payment milestones, completion target, before-work photos and progress evidence are recorded with the job. If documented work or a milestone cannot be supported, unreleased funds can be paused and reviewed under the signed agreement and payment terms." },
  { topic: "trust", question: "What evidence is used for a progress payment?", answer: "Contractors provide milestone evidence, including work-progress photos and any required notes. Homeowners can review it, and Operations can compare it with the original job record before a protected milestone is approved." },
  { topic: "trust", question: "Are reviews and ratings trustworthy?", answer: "Only homeowners attached to a completed JobLink job can provide a completed-job review. JobLink Score uses verified job records, while Quote Rating measures how consistently an opening price range matches the finalized on-site quote." },
];
...
export default function Home() {
  ...
  const [demoLifecycleProgress, setDemoLifecycleProgress] = useState<Record<number, DemoLifecycleProgress | undefined>>({});
  ...
  const releasedPayoutRecords = paymentRecords.filter((payment) => ["demo_partially_released", "demo_released"].includes(payment.status));
  const releasedPayoutCents = releasedPayoutRecords.reduce((total, payment) => total + payment.contractorPayoutCents, 0);
  const displayedReleasedPayoutCents = releasedPayoutCents || contractorProfile?.demoMetrics?.releasedPayoutCents || 0;
  const displayedReleasedPayoutCount = releasedPayoutRecords.length || (contractorProfile?.demoMetrics?.completedJobs ? contractorProfile.demoMetrics.completedJobs : 0);
  ...
  async function refreshGeneratedDocuments() {
    try {
      const scope = view === "contractor" ? "contractor" : "homeowner";
      const response = await fetch(`/api/documents?scope=${scope}`);
      if (!response.ok) return;
  ...
  async function runDemoLifecycle(job: PersistedJob, action: DemoLifecycleAction) {
    ...
    try {
      ...
      const data = (await response.json().catch(() => ({}))) as { message?: string; error?: string; progress?: DemoLifecycleProgress };
      ...
      const message = data.message || "Demo records updated. No money moved.";
      if (data.progress) setDemoLifecycleProgress((current) => ({ ...current, [job.id]: data.progress }));
      setDemoLifecycleFeedback((current) => ({ ...current, [job.id]: { tone: "success", message } }));
      showNotice(message);
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unable to run the demo simulation";
      setDemoLifecycleFeedback((current) => ({ ...current, [job.id]: { tone: "error", message } }));
      showNotice(message);
    } finally {
  ...
  async function signServiceAgreement(document: GeneratedDocument, signerName: string, consent: boolean): Promise<ActionFeedback> {
    ...
    try {
      ...
      await refreshGeneratedDocuments();
      const message = data.fullySigned ? "Service agreement fully signed. Funding and a scheduled start are now the remaining steps." : "Signature recorded. JobLink notified the other party to review and sign.";
      showNotice(message);
      return { tone: "success", message };
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unable to record this signature";
      showNotice(message);
  ...
  async function submitPaymentProof(payment: PaymentRecord, milestone: PaymentMilestone, proofNote: string, files: File[]) {
    setPaymentWorkflowAction(`proof-${milestone.id}`);
    try {
      if (files.length) {
        const form = new FormData();
        form.append("stage", "progress");
        form.append("milestoneId", String(milestone.id));
        files.slice(0, 5).forEach((file) => form.append("files", file));
        const uploadResponse = await fetch(`/api/jobs/${payment.jobId}/attachments`, { method: "POST", body: form });
        const uploadData = (await uploadResponse.json().catch(() => ({}))) as { error?: string };
        if (!uploadResponse.ok) throw new Error(uploadData.error || "Unable to upload progress photos");
      }
      ...
      await refreshPaymentRecords();
      if (roomJob?.id === payment.jobId) await openJobRoom(roomJob);
  ...
  return (
    <main>
        ...
        <>
          ...
          <section className="how-section section-wrap">
            <div className="how-intro">
              ...
              <button className="link-button" onClick={() => go("how")}>See the full process <span>→</span></button>
            </div>
          ...
          <section className="payment-promise section-wrap">
            <div><p className="section-label">Payment protection</p><h2>Documented work.<br /><em>Protected payments.</em></h2></div>
            <div><p>For live payments, JobLink is designed to use a licensed third-party payment provider. The signed agreement, documented scope and approved milestones create the record behind each release - so funds are tied to the work both parties agreed to.</p><p className="payment-promise-note">This version demonstrates the complete payment workflow. It never charges a card or moves money.</p><button className="primary-action" onClick={() => go("how")}>See how protection works →</button></div>
          </section>
          ...
          <section className="proof-section">
            <div className="proof-copy">
              ...
              <div className="rating-explainer">
                <article><b>JobLink Score</b><p>Built from review quality, accepted-job completion, required reviews and on-time completion against the finalized completion target.</p></article>
                <article><b>Quote Rating</b><p>Recognizes contractors whose finalized prices stay aligned with the honest range they first provide, helping reliable estimates rank above artificially low opening bids.</p></article>
              </div>
            </div>
          ...
          <div className="account-layout">
            ...
            <div className="account-content">
              ...
              {accountTab === "payments" && <>
                ...
                <DemoLifecycleControls jobs={demoEligibleJobs} documents={generatedDocuments} payments={paymentRecords} actionId={demoLifecycleAction} feedback={demoLifecycleFeedback} progress={demoLifecycleProgress} onRun={(job, action) => void runDemoLifecycle(job, action)} />
                ...
                <div className="payment-summary"><article><span>Accepted job value</span><b>${(paymentRecords.reduce((total, payment) => total + payment.subtotalCents, 0) / 100).toLocaleString()}</b><small>Contractor prices recorded</small></article><article><span>Demo JobLink fees</span><b>${(paymentRecords.reduce((total, payment) => total + payment.customerFeeCents, 0) / 100).toLocaleString()}</b><small>Simulated 3% customer fee</small></article><article><span>Simulated payments</span><b className="card-ending">{releasedPayoutRecords.length}</b><small>No card charged · no funds moved</small></article></div>
              ...
              {jobView === "quotes" && contractorQuotes.some((quote) => quote.status === "agreement_pending") && <section className="onsite-request-panel agreement-signing-panel"><div><p className="aside-label">Service agreement</p><h2>Sign before this job is booked.</h2><p>The homeowner has selected this final quote for the agreement stage. Complete your demo signature in the agreement form directly below. Once both parties sign, the homeowner can accept and fund the final quote.</p></div><div className="onsite-request-list">{contractorQuotes.filter((quote) => quote.status === "agreement_pending").map((quote) => <article key={quote.id}><div><small>{quote.externalId} · agreement pending</small><b>{quote.title}</b><span>Final quote {"$"}{(quote.amountCents / 100).toLocaleString()} · contractor signature required</span></div><em>Signing form below</em></article>)}</div></section>}
                <div className="agreement-signature-workflows">{generatedDocuments.filter((document) => document.documentType === "service_agreement").map((document) => <div key={`agreement-photo-${document.id}`}>{document.viewerRole === "homeowner" && <BeforeWorkPhotoUpload jobId={document.jobId} photoCount={document.preWorkPhotoCount ?? 0} onUploaded={() => void refreshGeneratedDocuments()} />}<ImageThumbnailStrip images={document.preWorkPhotos ?? []} label="Before-work photos on this agreement" /><AgreementSignatureWorkflow document={document} actionId={documentSigningId} onSign={signServiceAgreement} /></div>)}</div>
      ...
      {view === "how" && (
        <section className="how-page">
          <header className="how-page-hero">
            <p className="section-label light">How JobLink works</p>
            <h1>From first request<br />to <em>finished work.</em></h1>
            <p>JobLink keeps quotes, on-site verification, the finalized agreement, payment protection and job evidence in one connected record.</p>
            <div><button className="primary-action" onClick={() => beginRequest()}>Post a job request →</button><button className="how-hero-link" onClick={() => go("contractor")}>Contractor workspace →</button></div>
          </header>
          <section className="how-audiences">
            <article className="how-process-card"><p className="section-label">For homeowners</p><h2>Hire with the full picture.</h2><ol><li><b>Post your request</b><p>Answer a few guided questions, add photos and send a clear job brief to qualified local professionals.</p></li><li><b>Compare preliminary estimates</b><p>Review contractor profiles, ratings, opening price ranges, availability and estimated work duration. These estimates are non-binding.</p></li><li><b>Choose on-site verification</b><p>Ask one or more contractors to visit. You are not committed until you review a finalized quote and agreement.</p></li><li><b>Review the finalized agreement</b><p>See the confirmed scope, materials, start and completion dates, payment milestones and documented responsibilities before you select a contractor.</p></li><li><b>Sign, fund and track</b><p>Upload before-work photos, complete the agreement and use the protected payment workflow. Milestones are supported by progress evidence and approvals.</p></li><li><b>Confirm completion and review</b><p>Review the completed work and leave a required verified review before the final release is completed.</p></li></ol></article>
            <article className="how-process-card contractor"><p className="section-label">For contractors</p><h2>Win the right work.</h2><ol><li><b>Build a verified business profile</b><p>Operations reviews your business, insurance, licences and the services your company is qualified to provide.</p></li><li><b>Receive relevant opportunities</b><p>Only requests matching your approved services, territory and availability appear in your workspace.</p></li><li><b>Provide a realistic opening range</b><p>Submit a transparent initial range, timing and message. Quote Rating recognizes opening estimates that remain accurate after the on-site visit.</p></li><li><b>Verify the work on site</b><p>Confirm measurements, materials and scope, then provide the finalized quote, start date, completion target and milestone plan.</p></li><li><b>Document progress</b><p>Upload required progress evidence for each milestone so homeowners and Operations can review the work against the signed agreement.</p></li><li><b>Build verified reputation</b><p>Completed jobs, homeowner reviews, quote accuracy and on-time completion contribute to a stronger JobLink Score.</p></li></ol></article>
          </section>
          <section className="how-payment-callout"><div><p className="section-label light">Licensed payment protection</p><h2>Funds follow the<br />agreed work.</h2></div><div><p>For live payments, JobLink is designed to use a licensed third-party payment provider. Funds are connected to the signed agreement and released only by the agreed payment plan after documented progress, homeowner approval and Operations review where needed.</p><p>If work or a milestone cannot be supported by the job record, unreleased funds can be paused while the issue is reviewed under the signed agreement and payment terms. The current payment workflow is a clearly labelled demo: it never charges a card or moves money.</p><button onClick={() => go("trust")}>Explore trust and safety →</button></div></section>
          <section className="how-page-cta"><h2>Ready to start with clarity?</h2><p>Post a free request or set up a verified contractor profile.</p><div><button className="primary-action" onClick={() => beginRequest()}>Post a request →</button><button className="secondary-action" onClick={() => setAccountGatewayOpen(true)}>Create an account</button></div></section>
        </section>
      )}

      {view === "trust" && (
        <section className="trust-page">
          <div className="trust-hero"><div><p className="section-label light">Trust & safety</p><h1>Confidence is<br />built into <em>every job.</em></h1><p>Verified contractor profiles, signed agreements, protected payment records and completed-job reviews stay connected to the people and work they cover.</p></div><div className="trust-seal"><span>✓</span><b>JobLink<br />Recorded</b><small>Every booked job</small></div></div>
          ...
          <div className="trust-payment-notice"><b>Licensed third-party payment protection</b><p>For live payments, JobLink is designed to use a licensed third-party payment provider. Funds follow the signed agreement and documented milestones; this demo simulates the workflow and never charges a card or moves money.</p></div>
          <div className="protection-grid"><article className="protection-main"><p className="section-label light">Payment protection</p><h2>Your money follows<br />verified work.</h2><p>JobLink records the agreed scope, materials, schedule, milestones and homeowner-approved changes before funds are released. Payments are processed through JobLink so progress can be documented, reviewed and tied to the work both parties agreed to.</p><p>If a contractor does not provide the documented services or cannot support a required milestone, JobLink can pause unreleased funds while Operations reviews the job record. Amounts not earned for verified work can be returned to the homeowner under the signed agreement and payment terms.</p><div className="protection-flow"><span><b>1</b>Sign agreement</span><i>→</i><span><b>2</b>Fund securely</span><i>→</i><span><b>3</b>Release by milestone</span></div></article><aside><p className="aside-label">Workflow includes</p><ul><li><span>✓</span>Protected payment collection</li><li><span>✓</span>Documented scope and change decisions</li><li><span>✓</span>Progress evidence and approvals</li><li><span>✓</span>Operations dispute review</li><li><span>✓</span>Contractor accountability for agreed work</li></ul></aside></div>
          <div className="trust-score-explain"><div><p className="section-label">Verified reputation</p><h2>Reviews and ratings<br />you can trust.</h2><p>Only homeowners with a completed JobLink job can leave a review. JobLink Score reflects verified work quality, completed jobs, required reviews and on-time completion. Quote Rating reflects how consistently a contractor’s final on-site price matches the range they gave you first - so a low price alone never decides who ranks highest.</p></div><div className="signal-grid"><article><b>ID</b><span>Operations-approved profile</span></article><article><b>JOB</b><span>Completed accepted job record</span></article><article><b>REV</b><span>Verified homeowner review</span></article><article><b>JL</b><span>JobLink Score: quality and reliability</span></article><article><b>QT</b><span>Quote Rating: price-range accuracy</span></article><article><b>CASE</b><span>Disputes and resolutions</span></article></div></div>
        ...
        <section className="pro-shell">
          ...
          {proTab === "overview" && (
            <div className="pro-page">
              ...
              <div className="pro-metrics">
                ...
                <article><span>Released demo payouts</span><b>${(displayedReleasedPayoutCents / 100).toLocaleString()}</b><small>{displayedReleasedPayoutCount} released {displayedReleasedPayoutCount === 1 ? "job" : "jobs"}</small></article>
          ...
          {proTab === "jobs" && (
            <div className="pro-page">
              ...
              {jobView === "quotes" && generatedDocuments.filter((document) => document.documentType === "service_agreement" && document.status !== "fully_signed").map((document) => <AgreementSignatureWorkflow key={`contractor-agreement-${document.id}`} document={document} actionId={documentSigningId} onSign={signServiceAgreement} />)}
              <div className="pro-panel payment-panel"><div><p className="aside-label">Demo payment ledger</p><h2>${(displayedReleasedPayoutCents / 100).toLocaleString()}</h2><span>{displayedReleasedPayoutCount} released {displayedReleasedPayoutCount === 1 ? "job" : "jobs"} · no funds moved</span></div><button onClick={() => setProTab("business")}>Demo plan settings →</button></div>
            </div>
          )}
          ...
          {quoteJob && (
            <div className="quote-overlay" role="dialog" aria-modal="true" aria-labelledby="quote-title">
              <div className="quote-drawer">
                ...
                <div className="quote-breakdown"><div className="total"><span>Your submitted initial range</span><b>${Number(quoteMinAmount || 0).toLocaleString()}–${Number(quoteMaxAmount || 0).toLocaleString()}</b></div><p>Use a range you can reasonably support. Quote Rating recognizes estimates that remain useful and accurate after on-site verification, helping homeowners compare realistic pricing rather than artificially low opening bids. Any verified scope change belongs in the finalized quote.</p></div>
      ...
      {roomJob && roomAttachments.length > 0 && <aside className="job-media-tray" aria-label="Job photos and videos"><p className="aside-label">Job photos & video</p><ImageThumbnailStrip images={roomAttachments.filter((attachment) => attachment.kind === "image")} label="Photo evidence" /><div>{roomAttachments.filter((attachment) => attachment.kind === "video").map((attachment) => <a key={attachment.id} href={attachment.url} target="_blank" rel="noreferrer"><span>VID</span><p><b>{attachment.filename}</b><small>{(attachment.sizeBytes / 1024 / 1024).toFixed(1)} MB</small></p><em>↗</em></a>)}</div></aside>}
      ...
      {selectedOperationCase?.details.documents && selectedOperationCase.details.documents.some((document) => document.contentType?.startsWith("image/")) && <aside className="verification-thumbnail-tray"><p className="aside-label">Verification image evidence</p><div>{selectedOperationCase.details.documents.filter((document) => document.contentType?.startsWith("image/")).map((document) => <a key={document.id} href={`/api/contractor-verification/${document.id}`} target="_blank" rel="noreferrer"><img src={`/api/contractor-verification/${document.id}`} alt={document.filename} /><span>{document.filename}</span></a>)}</div></aside>}
      ...
      <footer>
        ...
        <div><button onClick={() => go("how")}>How it works</button><button onClick={() => go("trust")}>Trust & safety</button><button onClick={() => setAccountGatewayOpen(true)}>Create an account</button><button onClick={() => go("help")}>Help</button><button onClick={() => { setAccountGatewayOpen(true); setAccountActionError(""); }}>Operations login</button></div>
