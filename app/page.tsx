      ...
      {view === "contractor" && (
        <section className="pro-shell">
          <div className="pro-banner">
            <div className="pro-company-area"><div className="pro-company"><span>{(contractorProfile?.businessName || businessName).split(" ").slice(0,2).map((word) => word[0]).join("")}</span><div><b>{contractorProfile?.businessName || businessName}</b><small>{contractorProfile?.verificationStatus === "verified" ? "Verified business" : "Business profile"} · {contractorProfile?.homeBase || homeBase}</small></div></div>{demoContractorCompanies.length > 1 && <label className="demo-company-switcher"><span>Demo company view · switch company</span><select value={selectedDemoContractorEmail} disabled={demoCompanySwitching} onChange={(event) => void switchDemoContractor(event.target.value)}>{demoContractorCompanies.map((company) => <option key={company.ownerEmail} value={company.ownerEmail}>{company.businessName} · {company.primaryService}</option>)}</select></label>}</div>
