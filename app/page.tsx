...
export default function Home() {
  ...
  function quoteWorkflowLabel(quote: PersistedQuote) {
    ...
    if (quote.status === "final_quote_ready") return "Choose final quote for agreement";
          ...
          <div className="match-layout">
            ...
            {savedRequestId && savedQuotes.length > 0 && savedQuotes.map((quote, index) => (
                <article className={`contractor-card ${index === 0 ? "featured" : ""}`} key={quote.id}>
                  <div className="rank">0{index + 1}</div><div className="pro-logo">{quote.contractorName.split(" ").slice(0, 2).map((word) => word[0]).join("")}</div><div className="pro-main"><div className="pro-title"><div><span className="match-badge">{["final_quote_ready", "agreement_pending"].includes(quote.status) ? "Final verified quote" : index === 0 ? "Lowest initial estimate" : "Verified contractor"}</span><h2>{quote.contractorName} <small>✓</small></h2></div></div><div className="pro-facts"><span>Profile approved by Operations</span><span>{quoteWorkflowLabel(quote)}</span></div><div className="quote-row"><div><small>{["final_quote_ready", "agreement_pending"].includes(quote.status) ? "On-site visit" : "Available"}</small><b>{["onsite_scheduled", "final_quote_ready", "agreement_pending"].includes(quote.status) ? scheduledStartLabel(quote.onsiteVisitAt) : quote.availableAt}</b></div><div><small>{["final_quote_ready", "agreement_pending"].includes(quote.status) ? "Final quote" : "Initial estimate"}</small><b className="quote-price">${(quote.amountCents / 100).toLocaleString()}</b></div></div>{!["final_quote_ready", "agreement_pending"].includes(quote.status) && <div className="quote-schedule-estimate"><span><small>Estimated start</small><b>{estimatedDateLabel(quote.estimatedStartAt)}</b></span><span><small>Work duration</small><b>{quoteCompletionLabel(quote.message)}</b></span></div>}<div className="contractor-selection-facts"><span><small>Verified rating</small><b>{quote.contractor?.averageRating ? `${quote.contractor.averageRating.toFixed(1)} ★` : "New to JobLink"}</b></span><span><small>Verified jobs</small><b>{quote.contractor?.reviewCount ?? 0} completed</b></span><span><small>Experience</small><b>{quote.contractor?.yearsInBusiness ? `${quote.contractor.yearsInBusiness} years` : "Not provided"}</b></span><span><small>Approved work</small><b>{quote.contractor?.approvedServices?.join(", ") || quote.contractor?.primaryService || "Verified service"}</b></span></div><p className="contractor-comparison-note">{quote.contractor?.about || "Operations has verified this company’s business profile, insurance evidence and applicable licence information."}</p><p className="quote-note">✓ {["final_quote_ready", "agreement_pending"].includes(quote.status) ? quote.workDescription : quote.message}</p>{["final_quote_ready", "agreement_pending"].includes(quote.status) && <div className="final-quote-summary"><b>Materials</b><p>{quote.materials}</p><b>Measurements</b><p>{quote.measurements}</p><div><span>Deposit ${(quote.depositCents || 0) / 100}</span><span>Progress ${(quote.progressCents || 0) / 100}</span><span>Completion ${(quote.completionCents || 0) / 100}</span></div>{quote.finalOptions?.length ? <section className="homeowner-final-options"><b>Other finalized options</b><p>Choose one option to prepare its agreement. Both parties must sign before it can be accepted.</p>{quote.finalOptions.map((option) => <article key={option.id}><div><strong>{option.title}</strong><p>{option.description}</p><small>Deposit ${(option.depositCents / 100).toLocaleString()} · Progress ${(option.progressCents / 100).toLocaleString()} · Completion ${(option.completionCents / 100).toLocaleString()}</small></div><button className="secondary-action" disabled={savedQuotesStatus === "loading" || quote.status === "agreement_pending"} onClick={() => void decideFinalQuote(quote, "accept_final", option.id)}>Prepare agreement for ${(option.amountCents / 100).toLocaleString()}</button></article>)}</section> : null}</div>}<div className="card-actions"><button className="secondary-action" onClick={() => { setStep(3); go("request"); }}>Review request</button>{quote.status === "final_quote_ready" ? <><button className="secondary-action" onClick={() => void decideFinalQuote(quote, "decline_final")}>Decline</button><button className="primary-action" disabled={savedQuotesStatus === "loading"} onClick={() => void decideFinalQuote(quote, "accept_final")}>Prepare agreement & sign →</button></> : quote.status === "agreement_pending" ? <><button className="secondary-action" onClick={() => { setAccountTab("documents"); go("account"); }}>Review agreement</button>{generatedDocuments.some((document) => document.documentType === "service_agreement" && document.quoteId === quote.id && document.status === "fully_signed") ? <button className="primary-action" disabled={savedQuotesStatus === "loading"} onClick={() => void decideFinalQuote(quote, "accept_final")}>Accept signed quote & fund job →</button> : <button className="primary-action" disabled>Awaiting both demo signatures</button>}</> : <button className="primary-action" disabled={savedQuotesStatus === "loading" || quote.status !== "submitted"} onClick={() => void acceptSavedQuote(quote)}>{quoteWorkflowLabel(quote)}</button>}</div></div>
                </article>
              ))}
