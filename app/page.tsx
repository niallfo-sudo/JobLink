...
type AdminTab = "overview" | "verification" | "fraud" | "disputes" | "team";
type PersistedJob = { id: number; externalId: string; category: string; title: string; description?: string; size?: string; timeline?: string; postalCode?: string; emergency?: boolean; status: string; budget: string; createdAt: string | number };
...
const opportunities: Opportunity[] = [
  { id: "JL-2194", service: "Drywall", title: "Repair water-damaged basement ceiling", distance: "6.2 km", budget: "$1,800–$2,400", timing: "This week", match: 98, posted: "4 min ago", details: "Approx. 180 sq. ft. Remove damaged board, inspect insulation, replace, tape, mud and sand. Photos attached." },
  { id: "JL-2191", service: "Painting", title: "Paint main floor and stairwell", distance: "9.8 km", budget: "$3,000–$4,500", timing: "Within 2 weeks", match: 94, posted: "18 min ago", details: "Living room, dining room, hallway and open stairwell. Walls only; customer will select colours." },
  { id: "JL-2187", service: "Drywall", title: "Board and finish new garage", distance: "14.1 km", budget: "$4,500–$6,000", timing: "Flexible", match: 91, posted: "36 min ago", details: "Two-car detached garage, walls and ceiling. Insulation complete. Fire-rated board required on shared wall." },
];
...
export default function Home() {
  ...
  const [selectedProfile, setSelectedProfile] = useState<(typeof contractors)[number] | null>(null);
  ...
  async function loadOperations() {
    ...
    try {
      ...
      const data = (await response.json()) as { cases?: OperationsCase[]; stats?: typeof operationsStats; viewer?: { email?: string; displayName: string; role: string }; staff?: StaffMember[] };
  ...
  const availableOpportunities = useMemo(() => {
    ...
    const combined = [...liveOpportunities, ...opportunities.filter((job) => !liveIds.has(job.id))]
    ...
    return [...combined].sort((a, b) => opportunitySort === "nearest" ? Number.parseFloat(a.distance) - Number.parseFloat(b.distance) : opportunitySort === "newest" ? combined.indexOf(a) - combined.indexOf(b) : b.match - a.match);
  ...
  async function acceptSavedQuote(quote: PersistedQuote) {
    ...
    try {
      ...
      setAccepted(quote.contractorName);
  ...
  async function saveContractorProfile() {
    ...
    try {
      const response = await fetch("/api/contractor-profile", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ businessName, legalName, phone: businessPhone, about: businessAbout, primaryService, services: Array.from(new Set([primaryService, ...selectedServices])), homeBase, serviceRadiusKm: serviceRadius, teamSize: 4, emergencyAvailable, acceptingWork: true, plan: selectedPlan }),
      });
    ...
    <main>
        ...
        <>
          ...
          <section className="proof-section">
            <div className="proof-copy">
              ...
              <p>JobLink Trust Scores combine verified work history, punctuality, communication, licences, insurance and warranty claims.</p>
            </div>
            <div className="score-card">
              <div className="score-top"><span>JobLink Trust Score</span><b>96<small>/100</small></b></div>
        ...
        <section className="app-shell matches-shell">
          ...
          <div className="match-layout">
            <div className="contractor-list">
              ...
              {savedRequestId && savedQuotes.length > 0 && savedQuotes.map((quote, index) => (
                <article className={`contractor-card ${index === 0 ? "featured" : ""}`} key={quote.id}>
                  <div className="rank">0{index + 1}</div><div className="pro-logo" style={{ background: contractors[index % contractors.length].color }}>{quote.contractorName.split(" ").slice(0,2).map((word) => word[0]).join("")}</div><div className="pro-main"><div className="pro-title"><div><span className="match-badge">{index === 0 ? "Best value" : "Verified pro"}</span><h2>{quote.contractorName} <small>✓</small></h2></div><div className="trust-score"><b>{Math.max(88, 96 - index * 3)}</b><span>Trust score</span></div></div><div className="pro-facts"><span><b>★ {index === 0 ? "4.9" : "4.8"}</b> verified</span><span>Insured</span><span>Identity checked</span><span>Work history verified</span></div><div className="quote-row"><div><small>Available</small><b>{quote.availableAt}</b></div><div><small>Submitted quote</small><b className="quote-price">${(quote.amountCents / 100).toLocaleString()}</b></div></div><p className="quote-note">✓ {quote.message}</p><div className="card-actions"><button className="secondary-action" onClick={() => { setStep(3); go("request"); }}>Review scope</button><button className="primary-action" disabled={savedQuotesStatus === "loading" || (acceptedQuoteId !== null && acceptedQuoteId !== quote.id)} onClick={() => void acceptSavedQuote(quote)}>{quote.status === "accepted" ? "Selected ✓" : quote.status === "declined" ? "Another pro selected" : "Choose this pro"}</button></div></div>
                </article>
              ))}
              {(!savedRequestId || (savedQuotesStatus !== "loading" && savedQuotes.length === 0)) && matchedContractors.map((pro, index) => (
                <article className={`contractor-card ${index === 0 ? "featured" : ""}`} key={pro.name}>
                  ...
                  <div className="pro-main">
                    ...
                    <div className="card-actions"><button className="secondary-action" onClick={() => setSelectedProfile(pro)}>View profile</button><button className="primary-action" onClick={() => setAccepted(pro.name)}>{accepted === pro.name ? "Quote accepted ✓" : "Choose this pro"}</button></div>
                  </div>
                </article>
              ))}
            </div>
            <aside className="insight-card">
              ...
              <h3>{savedQuotes[0]?.contractorName ?? matchedContractors[0].name} is your strongest fit.</h3>
              <p>{savedQuotes.length ? `Their $${(savedQuotes[0].amountCents / 100).toLocaleString()} quote includes a verified scope and the earliest strong schedule match.` : `They’re 8 km away, have completed 42 similar ${category.toLowerCase()} jobs and respond 37% faster than average.`}</p>
              ...
              <button disabled={savedQuotesStatus === "loading" || Boolean(acceptedQuoteId)} onClick={() => savedQuotes[0] ? void acceptSavedQuote(savedQuotes[0]) : setAccepted(matchedContractors[0].name)}>{acceptedQuoteId ? "Best match selected ✓" : "Choose best match →"}</button>
            </aside>
          </div>
      ...
      {view === "tracking" && (
        <section className="app-shell tracking-shell">
          <div className="tracking-top">
            <div><p className="step-kicker">{trackingJob?.status === "booked" ? "Booked job" : "Active job"} · {trackingJob?.externalId ?? savedRequestId ?? "JobLink"}</p><h1>{trackingJob?.status === "booked" ? "Your professional is confirmed." : "Your job is underway."}</h1><p>{trackingJob?.category ?? category} · {trackingJob?.title ?? selectedJobType} with {acceptedSavedQuote?.contractorName ?? accepted ?? matchedContractors[0].name}</p></div>
          ...
          <div className="tracking-layout">
            <div className="live-card">
              ...
              <div className="arrival-panel"><span className="pulse-icon"><i /></span><div><small>{trackingJob?.status === "booked" ? "Booking confirmed" : "Crew is on the way"}</small><h2>{trackingJob?.status === "booked" ? (acceptedSavedQuote?.availableAt || "Schedule confirmed") : "Arriving in 14 minutes"}</h2><p>{acceptedSavedQuote?.contractorName ?? accepted ?? matchedContractors[0].name} · updates saved in Job Room</p></div><button onClick={() => trackingJob ? void openJobRoom(trackingJob) : openActiveJobRoom()}>Message professional</button></div>
            </div>
            <aside className="job-detail-card">
              <p className="aside-label">Today’s progress</p>
              <ol className="timeline">{roomEvents.length ? roomEvents.map((event, index) => <li key={event.id} className={index < roomEvents.length - 1 ? "complete" : "current"}><span>{index < roomEvents.length - 1 ? "✓" : <i />}</span><div><b>{event.label}</b><small>{new Date(event.createdAt).toLocaleString()}</small></div></li>) : <><li className="complete"><span>✓</span><div><b>Quote accepted</b><small>Saved to JobLink</small></div></li><li className="current"><span><i /></span><div><b>Booking confirmed</b><small>{acceptedSavedQuote?.availableAt || "Schedule pending"}</small></div></li><li><span>3</span><div><b>Crew dispatch</b><small>Next update</small></div></li><li><span>4</span><div><b>Work and cleanup</b></div></li><li><span>5</span><div><b>Final walkthrough</b></div></li></>}</ol>
            </aside>
          </div>
          <div className="tracking-bottom">
            ...
            <div><p className="aside-label">Agreed quote</p><h3>$2,280</h3><span>Payment protected by JobLink</span></div>
            <div><p className="aside-label">Documents</p><button onClick={() => openDocument("service_agreement")}>View contract ↗</button><button onClick={() => openDocument("accepted_quote")}>View quote ↗</button></div>
          </div>
        </section>
      )}
      ...
      {view === "silent" && (
        <section className="silent-page">
          <div className="silent-intro"><p className="section-label light">JobLink Silent Mode</p><h1>Say the problem.<br /><em>Everything else happens.</em></h1><p>JobLink can create the request, gather qualified quotes and bring back one simple decision—without searching, calling or opening an app.</p><button onClick={() => setSilentStage(1)}>Start voice demo →</button></div>
          <div className="voice-demo">
            <div className={`voice-orb stage-${silentStage}`}><i /><span>{silentStage === 0 ? "◉" : silentStage === 1 ? "≈" : silentStage === 2 ? "✦" : "✓"}</span></div>
            <div className="voice-copy">
              {silentStage === 0 && <><small>Ready when you are</small><h2>“Find someone to fix my furnace tomorrow.”</h2><p>Tap Start voice demo to see JobLink handle the request.</p></>}
        ...
        <section className="admin-shell">
          ...
          <div className="admin-layout"><aside className="admin-nav"><p>Workspace</p><button className={adminTab === "overview" ? "selected" : ""} onClick={() => setAdminTab("overview")}><span>OV</span>Overview <b>{operationsStats.openCases}</b></button><button className={adminTab === "verification" ? "selected" : ""} onClick={() => setAdminTab("verification")}><span>VR</span>Verification <b>{operationsCases.filter((item) => item.caseType === "verification" && !["resolved","dismissed"].includes(item.status)).length}</b></button><button className={adminTab === "fraud" ? "selected" : ""} onClick={() => setAdminTab("fraud")}><span>FR</span>Fraud review <b>{operationsCases.filter((item) => item.caseType === "fraud" && !["resolved","dismissed"].includes(item.status)).length}</b></button><button className={adminTab === "disputes" ? "selected" : ""} onClick={() => setAdminTab("disputes")}><span>DS</span>Disputes <b>{operationsCases.filter((item) => item.caseType === "dispute" && !["resolved","dismissed"].includes(item.status)).length}</b></button>{operationsViewer?.role === "admin" && <button className={adminTab === "team" ? "selected" : ""} onClick={() => setAdminTab("team")}><span>TM</span>Team <b>{operationsStaff.length}</b></button>}<div className="admin-system"><span><i /></span><div><b>Protected employee access</b><small>Changes are saved and attributed</small></div></div></aside><div className="admin-content">
            ...
            {operationsStatus !== "loading" && operationsStatus !== "error" && <>
              <div className="admin-title"><div><p className="step-kicker">Employee operations · {new Date().toLocaleDateString("en-CA", { weekday: "long", month: "long", day: "numeric" })}</p><h1>{adminTab === "overview" ? "Marketplace operations." : adminTab === "verification" ? "Verification queue." : adminTab === "fraud" ? "Fraud review." : adminTab === "team" ? "Operations team." : "Dispute resolution."}</h1><p>{adminTab === "overview" ? "Live workload, marketplace records and cases needing action." : adminTab === "team" ? "Grant or remove protected Operations access for JobLink employees." : `Review, document and resolve every ${adminTab === "disputes" ? "dispute" : adminTab} case.`}</p></div><span className="operations-live"><i /> Database live</span></div>
              ...
              {adminTab !== "team" && <><div className="operations-toolbar"><label><span>Search cases</span><input value={operationSearch} onChange={(event) => setOperationSearch(event.target.value)} placeholder="Case ID, business, job or assignee" /></label><label><span>Status</span><select value={operationStatusFilter} onChange={(event) => setOperationStatusFilter(event.target.value)}><option value="active">Active cases</option><option value="all">All cases</option><option value="open">Open</option><option value="in_review">In review</option><option value="waiting">Waiting</option><option value="resolved">Resolved</option><option value="dismissed">Dismissed</option></select></label><button onClick={() => { setOperationSearch(""); setOperationStatusFilter("active"); }}>Clear filters</button></div>
        ...
        <section className="account-shell">
          ...
          <div className="account-layout">
            <aside className="account-sidebar">
              ...
              <div className="account-safety"><span>✓</span><div><b>JobLink protected</b><p>Your active job and payment are covered.</p></div></div>
            </aside>
            <div className="account-content">
              ...
              {accountTab === "payments" && <>
                <div className="account-section-head"><div><p className="aside-label">Money</p><h2>Payments</h2></div><span className="protected-badge">Protected by JobLink</span></div>
      ...
      {view === "trust" && (
        <section className="trust-page">
          <div className="trust-hero"><div><p className="section-label light">Trust & safety</p><h1>Confidence is<br />built into <em>every job.</em></h1><p>Identity checks, verified credentials, protected payments and real performance history—working before, during and after the job.</p></div><div className="trust-seal"><span>✓</span><b>JobLink<br />Protected</b><small>Every booked job</small></div></div>
      ...
      {view === "contractor" && (
        <section className="pro-shell">
          <div className="pro-banner">
            ...
            <div className="pro-banner-actions"><button className="pro-availability" disabled={contractorProfile?.verificationStatus !== "verified"} onClick={() => contractorProfile && updateContractorProfile({ acceptingWork: !contractorProfile.acceptingWork })}><span className={contractorProfile?.acceptingWork === false ? "paused" : ""}><i /></span><div><b>{contractorProfile?.verificationStatus !== "verified" ? "Matching awaiting verification" : contractorProfile?.acceptingWork === false ? "Matching paused" : "Accepting new work"}</b><small>{contractorProfile?.verificationStatus !== "verified" ? "Operations approval required" : "Click to change availability"}</small></div></button><button onClick={() => { setOnboardingStep(0); go("onboarding"); }}>{contractorProfile ? "Edit profile" : "Complete onboarding"}</button></div>
          </div>
          ...
          {proTab === "overview" && (
            <div className="pro-page">
              ...
              <div className="pro-metrics">
                ...
                <article><span>Quotes awaiting reply</span><b>{quoteSent ? 1 : 0}</b><small>Submitted through JobLink</small></article>
            ...
            <div className="pro-page">
              ...
              <div className="jobs-board" aria-hidden="true" style={{ display: "none" }}>
                <article className="active-job-feature">
                  ...
                  <div className="active-job-actions"><button onClick={() => setProTab("inbox")}>Message customer</button><button onClick={() => showNotice("Use the milestone controls above to update live progress.")}>Update progress</button><button className="primary-action" onClick={() => setChangeOrderOpen(true)}>{changeOrderSent ? "Change order sent ✓" : "Create change order"}</button></div>
                </article>
            ...
            <div className="pro-page inbox-page">
              ...
              <div className="inbox-layout">
                ...
                <div className="chat-panel">
                  ...
                  <form className="message-composer" onSubmit={(event) => { event.preventDefault(); if (chatMessage.trim()) { setSentMessages([...sentMessages, chatMessage.trim()]); setChatMessage(""); } }}><button type="button" onClick={() => showNotice("Open the live Job Room to attach job photos or documents.")}>+</button><input value={chatMessage} onChange={(event) => setChatMessage(event.target.value)} placeholder="Write a message…" /><button type="submit">Send →</button></form>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="pro-page">
              ...
              <div className="business-grid">
                ...
                <div className="plans-area">
                  ...
                  <div className="fee-note"><span>◎</span><div><b>You keep 100% of your quoted labour price.</b><p>When customers pay through JobLink, they cover a separate booking and protection fee. We never charge you per lead.</p></div></div>
                </div>
              </div>
            </div>
          )}
