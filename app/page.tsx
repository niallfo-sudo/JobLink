...
export default function Home() {
  ...
  async function clearAllJobPostings() {
    ...
    const confirmation = window.prompt("This permanently clears every job posting, quote, Job Room message, document, demo payment record and uploaded job file. Contractor and homeowner accounts will remain. Type CLEAR JOBS to continue.");
    if (confirmation !== "CLEAR JOBS") {
      if (confirmation !== null) showNotice("Marketplace reset cancelled. Type CLEAR JOBS exactly to confirm.");
      return;
    }
    setOperationsResetStatus("saving");
    try {
      const response = await fetch("/api/operations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "clear_job_postings", confirmation }) });
