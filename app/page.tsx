...
const helpFaqsRaw = [
  ...
  { topic: "trust", question: "How is the JobLink Score calculated?", answer: "The JobLink Score reflects verified work, not unverified public reviews. It is weighted from verified review quality (55%), completion of accepted jobs (30%), and completion of required homeowner reviews (15%). The score updates as completed jobs and verified reviews are recorded." },
...
export default function Home() {
  ...
  const requestStepValid = step === 0 ? scope.trim().length >= 5 : step === 1 ? size.trim().length >= 2 : step === 2 ? postalCode.trim().length >= 3 && (timeline !== "Custom" || customTimeline.trim().length >= 3) : true;
  ...
  const jobBrief = useMemo(
    () => {
      ...
      return `Looking for an experienced ${category.toLowerCase()} professional for ${selectedJobType.toLowerCase()}: ${scope.trim()}. The job covers ${size.trim()}.${detailText}${includeText} Preferred timing is ${requestTimeline.toLowerCase()}.${detailLevelText}${notesText}`;
    },
    [category, selectedJobType, scope, size, selectedJobDetails, quoteIncludes, requestTimeline, requestDetailLevel, additionalDetails],
  );
  const effectiveBrief = aiBrief?.description || jobBrief;

  useEffect(() => { setAiBrief(null); setAiBriefStatus("idle"); }, [category, selectedJobType, scope, size, selectedJobDetails, quoteIncludes, requestTimeline, requestDetailLevel, additionalDetails]);
  ...
  async function generateAiBrief() {
    ...
    try {
      const response = await fetch("/api/ai/request-brief", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ category, jobType: selectedJobType, scope, size, details: selectedJobDetails, includes: quoteIncludes, timeline: requestTimeline, notes: additionalDetails }) });
  ...
  async function submitQuote() {
    ...
    try {
      const response = await fetch(`/api/jobs/${quoteJob.numericId}/quotes`, {
        ...
        body: JSON.stringify({ minAmount: Number(quoteMinAmount), maxAmount: Number(quoteMaxAmount), message: quoteNote, availableAt: quoteAvailability, estimatedStartAt: quoteEstimatedStart, estimatedFinishAt: quoteEstimatedFinish, completionTimeframe: quoteCompletionTimeframe === "Custom" ? customQuoteCompletionTimeframe : quoteCompletionTimeframe, contractorName: contractorProfile?.businessName || businessName }),
      });
      const data = (await response.json()) as { error?: string };
          ...
          <section className="proof-section">
            <div className="proof-copy">
              ...
              <div className="rating-explainer">
                <article><b>JobLink Score</b><p>Built from verified review quality (55%), completion of accepted jobs (30%), and required-review completion (15%).</p></article>
          ...
          <div className="request-layout">
            <div className="form-panel">
              ...
              {step === 1 && <>
                ...
                {requestDetailLevel === "detailed" ? <><label className="field-label">Select everything a professional should know</label>
              ...
              {step === 3 && <>
                ...
                <div className="brief-card">
                  ...
                  <p>{effectiveBrief}</p>
                  <div className="brief-tags"><span>{selectedJobType}</span><span>◷ {requestTimeline}</span><span>⌂ {postalCode}</span>{isEmergency && <span>! Emergency</span>}{requestFiles.length > 0 && <span>▣ {requestFiles.length} visual{requestFiles.length === 1 ? "" : "s"}</span>}</div>
                </div>
