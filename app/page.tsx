    ...
    <main>
      ...
      {finalQuoteJob && <div className="quote-overlay" role="dialog" aria-modal="true" aria-labelledby="final-quote-title">
        <div className="quote-drawer final-quote-drawer">
          ...
          <label className="field-label" htmlFor="final-completion-date">Finalized completion target<input id="final-completion-date" type="date" min={earliestQuoteDate} value={finalCompletionDate} onChange={(event) => setFinalCompletionDate(event.target.value)} /><small>This promised date becomes part of your JobLink Score once the job is marked finished.</small></label>
          ...
          {!finalCompletionDate && <p className="quote-field-hint">Add a finalized completion target before sending this quote.</p>}
          <button className="send-quote" disabled={finalQuoteStatus === "saving" || !finalCompletionDate} onClick={() => void submitFinalQuote()}>{finalQuoteStatus === "saving" ? "Submitting…" : "Send final quote for review →"}</button>
        </div>
      </div>}
