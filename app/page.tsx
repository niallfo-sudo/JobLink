        ...
        <section className="pro-shell">
          ...
          {proTab === "jobs" && (
            <div className="pro-page">
              ...
              <div className="pro-panel payment-panel"><div><p className="aside-label">Demo payment ledger</p><h2>${(releasedPayoutCents / 100).toLocaleString()}</h2><span>{releasedPayoutRecords.length} released {releasedPayoutRecords.length === 1 ? "job" : "jobs"} · no funds moved</span></div><button onClick={() => setProTab("business")}>Demo plan settings →</button></div>
            </div>
          )}
